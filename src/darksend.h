...
// A helper object for signing messages from Tnodes
...
public:
     ...
     *     1) we have an active Tnode,
     *     2) we have a valid Tnode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid Tnode address
    bool CheckSignature(const CPubKey& pubKeyTnode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, tnode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 10000 TCR - checking if valid tnode)
...
private:
    ...
    // Keep track of the used Tnodes
    std::vector<CTxIn> vecTnodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // Tnode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Tnode to start the process
    ...
    /// Get Tnode updates about the progress of mixing
