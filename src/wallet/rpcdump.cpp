...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid tecracoin address or script");
    }
...
UniValue dumpprivkey_tecracoin(const JSONRPCRequest& request)
{
#ifndef UNSAFE_DUMPPRIVKEY
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            "dumpprivkey \"tecracoinaddress\"\n"
    ...
    if (request.params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + request.params[0].get_str(), request.params[1].get_str()))
    {
        std::string warning =
            ...
            "Tecracoin team members will never ask for this command's output and it is not needed for Znode setup or diagnosis!\n"
            "\n"
...
UniValue dumpwallet_tecracoin(const JSONRPCRequest& request)
{
#ifndef UNSAFE_DUMPPRIVKEY
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
