...
void SigmaDialog::on_mintButton_clicked()
{
    ...
    if (amount < smallestDenominationValue) {
        QMessageBox::critical(this, tr("Amount too small to mint"),
            tr("Amount to mint must not be lower than %1 TCR.").arg(formatAmount(smallestDenominationValue)),
    ...
    if (amount % smallestDenominationValue != 0) {
        ...
        auto reply = QMessageBox::question(
            ...
            tr("Amount to mint must be a multiple of 0.05 TCR. Do you want to mint %1 TCR?"
...
void SigmaDialog::coinControlChangeEdited(const QString& text)
{
    if (walletModel && walletModel->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid TecraCoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
...
void SigmaDialog::processSpendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::ExceedLimit:
        msgParams.first = tr("Transaction exceeds max number of spends (35) or value (500 TCR per transaction), please reduce the amount you wish to spend.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
