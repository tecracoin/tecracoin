...
/*
    At 15 signatures, 1/2 of the tnode network can be owned by
...
private:
    ...
    //track tnodes who voted with no txreq (for DOS protection)
...
public:
    ...
    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointTnodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
...
public:
    ...
    int CountVotes() const { return mapTnodeVotes.size(); }
