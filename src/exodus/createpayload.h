#ifndef TECRACOIN_EXODUS_CREATEPAYLOAD_H
#define TECRACOIN_EXODUS_CREATEPAYLOAD_H
...
std::vector<unsigned char> CreatePayload_SimpleSpend(uint32_t propertyId, uint8_t denomination, uint32_t group,
                                                    uint16_t groupSize, exodus::SigmaProof const &proof, secp_primitives::Scalar const &serial);
std::vector<unsigned char> CreatePayload_SimpleSpend(uint32_t propertyId, uint8_t denomination, uint32_t group,
...
#endif // TECRACOIN_EXODUS_CREATEPAYLOAD_H
