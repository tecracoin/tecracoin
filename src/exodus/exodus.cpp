...
static string exodus_address = "TDPHohNddqdG7ftxRKCJ1kEv79ZEU5nehW";

static const string exodus_mainnet = "TCYKRcon2bnMCPDCjJwatUspD7pG9tC83i";
static const string exodus_testnet = "GZxZbe6uKHA8HakBzueFPU5VZonpCejyN9";
static const string getmoney_testnet = "GVLyHwxrKUNKCAvVsAnyCmjJpSwj1cD62f";
...
{
    return !DISABLE_EXODUS && GetBoolArg("-exodus", false);
}
...
std::string exodus::strMPProperty(uint32_t propertyId)
{
    ...
    if (0x80000000 & propertyId) {
        ...
        switch (propertyId) {
            case EXODUS_PROPERTY_TCR: str = "TCR";
                break;
...
{
    if ((EXODUS_PROPERTY_TCR != propertyId) && !isTestEcosystemProperty(propertyId)) return true;
...
int input_mp_offers_string(const std::string& s)
{
    ...
    if (EXODUS_PROPERTY_TCR != prop_desired) return -1;
...
{
    if(DISABLE_EXODUS) {
        throw runtime_error("Exodus is disabled");
    }
