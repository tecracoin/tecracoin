 ...
 *
 * The log file can be specified via startup option "--exoduslogfile=/path/to/exodus.log",
...
static boost::filesystem::path GetLogPath()
{
    ...
    std::string strLogPath = GetArg("-exoduslogfile", "");
...
void InitDebugLogLevels()
{
    ...
    for (std::vector<std::string>::const_iterator it = debugLevels.begin(); it != debugLevels.end(); ++it) {
        ...
        if (*it == "ely") exodus_debug_ely = true;
        ...
        if (*it == "none" || *it == "all") {
            ...
            exodus_debug_ely = allDebugState;
