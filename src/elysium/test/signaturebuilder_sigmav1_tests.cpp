...
public:
    SignatureBuilderSigmaV1Setup()
        : address("TWouVuTvuM4xEXME1MsFvQwA1tSQihga8g"), proof(DefaultSigmaParams)
    {
...
BOOST_AUTO_TEST_CASE(verify)
{
    ...
    // invalid content
    signature = ECDSASignature::ParseCompact(context, validSig.data());
    builder = TestSignatureSigmaV1Builder(CBitcoinAddress("TSv7WJjRQV5daVGgCqj5WzW7zcSbNLFtdi"), 10, proof);
    BOOST_CHECK_EQUAL(false, builder.Verify(publicKey, signature));
