...
#ifndef KOMODO_VALIDATION_H
#define KOMODO_VALIDATION_H

#include <base58.h>
#include <key_io.h>
#include <wallet/wallet.h>

#include <komodo_defs_tecra.h>

extern const char ASSETCHAINS_SYMBOL[65];
...
#define STORE64L(x, y)                         \
    {                                          \
        (y)[7] = (uint8_t)(((x) >> 56) & 255); \
        (y)[6] = (uint8_t)(((x) >> 48) & 255); \
        (y)[5] = (uint8_t)(((x) >> 40) & 255); \
        (y)[4] = (uint8_t)(((x) >> 32) & 255); \
        (y)[3] = (uint8_t)(((x) >> 24) & 255); \
        (y)[2] = (uint8_t)(((x) >> 16) & 255); \
        (y)[1] = (uint8_t)(((x) >> 8) & 255);  \
        (y)[0] = (uint8_t)((x)&255);           \
...
{
    int32_t i, n = 0, prev;
    uint8_t serialized[sizeof(bits256) * 2];
    if (txn_count == 1)
        return (tree[0]);
    prev = 0;
    while (txn_count > 1) {
...
void komodo_notarized_update(int32_t nHeight, int32_t notarized_height, uint256 notarized_hash, uint256 notarized_desttxid, uint256 MoM, int32_t MoMdepth)
{
    ...
    static FILE* fp;
    CBlockIndex* pindex;
    struct notarized_checkpoint *np, N;
    long fpos;
    if (didinit == 0) {
        char fname[512];
        int32_t latestht = 0;
    ...
    if (fp != 0) {
        if (fwrite(np, 1, sizeof(*np), fp) == sizeof(*np))
            ...
            printf("error writing notarization to %d\n", (int32_t)ftell(fp));
    }
...
{
    int32_t notarized_height;
    uint256 zero, notarized_hash, notarized_desttxid;
    CBlockIndex* notary;
    CBlockIndex* pindex;
...
void komodo_voutupdate(int32_t txi, int32_t vout, uint8_t* scriptbuf, int32_t scriptlen, int32_t height, int32_t* specialtxp, int32_t* notarizedheightp, uint64_t value, int32_t notarized, uint64_t signedmask)
{
    ...
    int32_t MoMdepth, opretlen, len = 0;
    uint256 hash, desttxid, MoM;
...
void komodo_connectblock(CBlockIndex* pindex, CBlock& block)
{
    ...
    uint64_t signedmask;
    uint8_t scriptbuf[4096], pubkeys[64][33], scriptPubKey[35];
    uint256 zero;
    ...
    if (pindex != 0) {
        ...
        for (i = 0; i < txn_count; i++) {
            ...
            for (j = 0; j < numvins; j++) {
                if (i == 0 && j == 0)
                    continue;
                if (tx.vin[j].prevout.hash != zero && (scriptlen = gettxout_scriptPubKey(height, scriptPubKey, sizeof(scriptPubKey), tx.vin[j].prevout.hash, tx.vin[j].prevout.n)) == 35) {
            ...
            for (j = 0; j < numvouts; j++) {
                ...
                len = tx.vout[j].scriptPubKey.size();
                if (len >= (int32_t)sizeof(uint32_t) && len <= (int32_t)sizeof(scriptbuf)) {
                    memcpy(scriptbuf, &tx.vout[j].scriptPubKey[0], len);
                    komodo_voutupdate(i, j, scriptbuf, len, height, &specialtx, &notarizedheight, (uint64_t)tx.vout[j].nValue, notarized, signedmask);
                }
            }
            //if ( NOTARY_PUBKEY33[0] != 0 )
            //    printf(") ");
