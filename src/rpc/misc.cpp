...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"tecracoinaddress\"\n"
            "\nReturn information about the given tecracoin address.\n"
            "\nArguments:\n"
            "1. \"tecracoinaddress\"     (string, required) The tecracoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"tecracoinaddress\", (string) The tecracoin address validated\n"
            ...
            "  \"decompressedPubKey\" : \"hex\",   (string) The hex value of the raw decompressed public key (field present only if iscompressed=true)\n"
...
#ifdef ENABLE_WALLET
        // Case 1: tecracoin address and we have full public key:
...
UniValue tnsync(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are tecracoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) tecracoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"tecracoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"tecracoinaddress\"  (string, required) The tecracoin address to use for the signature.\n"
