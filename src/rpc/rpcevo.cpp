...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The TecraCoin address to send the collateral to.\n"
        },
        {"collateralHash",
        ...
        {"operatorPubKey",
            ...
            "                              It has to match the private key which is later used when operating the tnode.\n"
        },
        {"operatorReward",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The TecraCoin address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress",
            "%d. \"payoutAddress\"            (string, required) The TecraCoin address to use for tnode reward payments.\n"
        },
        {"proTxHash",
            "%d. \"proTxHash\"                (string, required) The hash of the initial ProRegTx.\n"
        },
        {"reason",
            "%d. reason                     (numeric, optional) The reason for tnode service revocation.\n"
        },
        {"votingAddress",
...
void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 TCR\n"
...
void protx_register_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "transaction output spendable by this wallet. It must also not be used by any other tnode.\n"
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TecraCoin address: ") + request.params[paramIdx + 6].get_str());
    }
...
void protx_update_service_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "of a tnode.\n"
            "If this is done for a tnode that got PoSe-banned, the ProUpServTx will also revive this tnode.\n"
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        throw std::runtime_error(strprintf("tnode with proTxHash %s not found", ptx.proTxHash.ToString()));
    }
    ...
    if (request.params.size() >= 6) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TecraCoin address: ") + request.params[5].get_str());
...
void protx_update_registrar_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "address of the tnode specified by \"proTxHash\".\n"
            "The owner key of the tnode must be known to your wallet.\n"
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            + GetHelpString(1, "proTxHash")
...
UniValue protx_update_registrar(const JSONRPCRequest& request)
{
    ...
    auto dmn = deterministicMNManager->GetListAtChainTip().GetMN(ptx.proTxHash);
    if (!dmn) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("tnode %s not found", ptx.proTxHash.ToString()));
    }
...
void protx_revoke_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates and sends a ProUpRevTx to the network. This will revoke the operator key of the tnode and\n"
            "put it into the PoSe-banned state. It will also set the service field of the tnode\n"
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TecraCoin address: ") + request.params[4].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceAddress.Get());
        ...
        // Using funds from previousely specified tnode payout address
        CTxDestination txDest;
...
void protx_info_help()
{
    throw std::runtime_error(
            ...
            "\nReturns detailed information about a deterministic tnode.\n"
            "\nArguments:\n"
            + GetHelpString(1, "proTxHash") +
            "\nResult:\n"
            "{                             (json object) Details about a specific deterministic tnode\n"
            "}\n"
            "\nExamples:\n"
...
void protx_diff_help()
{
    throw std::runtime_error(
            ...
            "\nCalculates a diff between two deterministic tnode lists. The result also contains proof data.\n"
            "\nArguments:\n"
...
#endif
            "  diff              - Calculate a diff and a proof between two tnode lists\n"
    );
}
