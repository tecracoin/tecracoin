...
struct ZerocoinTestingSetup : public TestingSetup {
    ZerocoinTestingSetup() : TestingSetup(CBaseChainParams::REGTEST, "1")
    {
        ...
        // Depending on miner's reward in the beginning blocks it's possible you would have to mine more blocks to get more coins, or else tests will fail
...
BOOST_AUTO_TEST_CASE(zerocoin_mintspend)
{
    ...
    for(int i = 0; i < 5; i++)
    {
        ...
        //Add 23 more blocks and verify that Mint can not be spent until 24 blocks verification
        for (int i = 0; i < 23; i++)
        {
            BOOST_CHECK_MESSAGE(!pwalletMain->CreateZerocoinSpendModel(stringError, "", denomination.c_str()), "Spend succeeded although not confirmed by 24 blocks");
            BOOST_CHECK_MESSAGE(stringError == "it has to have at least two mint coins with at least 24 confirmation in order to spend a coin", stringError + " - Incorrect error message");

            std::vector<CMutableTransaction> noTxns;
            CBlock b = CreateAndProcessBlock(noTxns, scriptPubKey);
        }
        BOOST_CHECK_MESSAGE(previousHeight + 23 == chainActive.Height(), "Block not added to chain");
