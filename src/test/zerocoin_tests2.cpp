...
struct ZerocoinTestingSetup2 : public TestingSetup {
    ZerocoinTestingSetup2() : TestingSetup(CBaseChainParams::REGTEST)
    {
        ...
        for (int i = 0; i < COINBASE_MATURITY + 49; i++)
        {
...
BOOST_AUTO_TEST_CASE(zerocoin_mintspend2)
{
    ...
    for (int d=1; d<=10; d*=10) {
        ...
        //Block COINBASE_MATURITY + 50 create 5 mints
        ...
        //Block COINBASE_MATURITY + 51, put 6 mints
        for(int i = 0; i < 6; i++)
        ...
        //Block COINBASE_MATURITY + 75, put 10 mints and one spend
        for(int i = 0; i < 10; i++)
            BOOST_CHECK_MESSAGE(pwalletMain->CreateZerocoinMintModel(stringError, denomination.c_str()), stringError + " - Create Mint failed");
        ...
        //20 spends in 20 blocks (we have 21 minted coins, so we can spend 20 of them)
        for(int i = 0; i < 20; i++) {
